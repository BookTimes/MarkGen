...
while True:
    ...
    t = input("\033[94m1)Help Menu\033[0m \n\033[92m2)Enter Marks \033[0m \nPlease enter required funtion number: ")
    if t == "1" :
        ...
        gaurdianName = input("Enter name of Gaurdian: ")
        remarks = input("Enter personal remark about the student  \033[92m within 120 characters \033[0m")
        disci = int(input("Enter a score from 1-5 rating student's discipline: "))
        s = input("Enter subjects taken by the student \033[36m separated by comas \033[0m : ")
        s = s.split(",")
        print("Marks can be entered in format [\033[95m SCORED MARKS/TOTAL MARK\033[0m]")
        for sub in s:
            mark = (input(f"Enter Marks of {sub}: "))
            itrnl = (input(f"Enter internals of {sub}: "))
            cltp = (input(f"Enter top marks of {sub}: "))
            clav = (input(f"Enter average marks of {sub}: "))
            temp = mark
            mark = eval(mark) * 100
            ...
            subject.append({"sub" : sub , "mark" : temp.split('/') , "internal":itrnl.split('/') , "classav" : clav.split('/') , "classtop":cltp.split('/') , 'grade' : grade , 'totmark' : int(eval(itrnl) + mark)})
            submark += (eval(temp),)
            topmark += (eval(cltp),)
            avmark += (eval(clav),)
    ...
    for i in subject:
        ...
        if len(warn) > 0:
            anre += f' <br>Need to work on {",".join(warn)} it\'s border line marks only the level of testing may increase in coming exams '
        tem = Template('''<tbody>
  ...
  <body>
    ...
    <div class="p-5">
     ...
     <div class="d-flex px-5 flex-column">
      ...
      >
      <span class="fs-5 fw-bolder"
        >Remarks:<br /><span class="fw-light ms-2">{remarks}</span></span
      >
    </div>
    <iframe
      src="graph.html"
